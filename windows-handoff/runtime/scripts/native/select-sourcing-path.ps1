param(
  [Parameter(Mandatory=$true)][ValidateSet('RAW_DIRECTORY','IMAGE_CACHE_DIRECTORY','IMAGE_DOWNLOAD_DIRECTORY','ANALYSIS_WORKBOOK')][string]$Kind,
  [string]$CurrentPath=''
)
...
if ($Kind -eq 'ANALYSIS_WORKBOOK') {
  ...
  $dialog.Description=if ($Kind -eq 'RAW_DIRECTORY') {'选择1688原始导出目录'} elseif ($Kind -eq 'IMAGE_DOWNLOAD_DIRECTORY') {'选择商品图片下载目录'} else {'选择1688图片缓存目录'}
