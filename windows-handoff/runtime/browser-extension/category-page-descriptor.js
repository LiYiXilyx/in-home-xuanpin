...
(() => {
  ...
  function parseCategoryPage(doc,url,now=()=>new Date()){
    ...
    const goodsId=n=>{try{const u=new URL(n.href,url);return u.pathname.match(/-g-(\d+)\.html/)?.[1]??u.searchParams.get('goods_id');}catch{return null;}};
    // Price is commonly a sibling of image/title anchors, inside one product
    // card. Never climb into a multi-product grid, cart, or page-wide container.
    const cardCurrency=link=>{
      const id=goodsId(link);if(!id)return false;
      if(/€|\bEUR\b/.test(text(link)))return true;
      for(let parent=link.parentElement,depth=0;parent&&depth<6;parent=parent.parentElement,depth++){
        if(!visible(parent)||['BODY','HTML','MAIN'].includes(parent.tagName))break;
        const ids=new Set(Array.from(parent.querySelectorAll('a[href*="-g-"],a[href*="goods_id="]')).map(goodsId).filter(Boolean));
        if(ids.size!==1||!ids.has(id))break;
        if(/€|\bEUR\b/.test(text(parent)))return true;
      }
      return false;
    };
    ...
    // Dropdown text can include all options. Read the selected Sort by line,
    // as the manual passive runner does; an unselected option is not evidence.
    const sort=nodes('button,[role="button"],[aria-selected="true"]').some(n=>{
      const selectedLine=String(n.innerText??n.textContent??'').split(/\r?\n/).map(normalize).find(line=>/^Sort by:/i.test(line));
      return selectedLine ? /^Sort by:\s*Top sales$/i.test(selectedLine)
        : n.getAttribute?.('aria-selected')==='true'&&/^Top sales$/i.test(text(n));
    });
    return validateDescriptor({descriptor_schema_version:1,page_url:url,page_type:'CATEGORY_LISTING',site_country:new URL(url).pathname.startsWith('/de-en/')?'DE':'',
      language:/^en(?:-|$)/i.test(doc.documentElement.lang)?'en':'',currency:cards.some(cardCurrency)?'EUR':'',sort_order:sort?'Top Sales':'',breadcrumbs,
