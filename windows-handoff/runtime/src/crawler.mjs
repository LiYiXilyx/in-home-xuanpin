...
import { openBrowserSession as openContext, connectOperatorSession as openExistingOperatorContext, closeBrowserSession as closeSession } from './browser/cdp-session.mjs';
import { findCurrentOperatorTemuPage } from './browser/operator-page.mjs';
import { handleChallenge } from './browser/challenge-handler.mjs';
import { waitForOperatorConfirmation } from './browser/manual-gate.mjs';
import {
  finishRun,
...
async function promptEnter(message, config) {
  return waitForOperatorConfirmation({ config, message, label: '运营确认' });
}
...
async function resolveTransientProductProblem(page, config, label, expectedProductUrl) {
  ...
  for (let attempt = 1; attempt <= retryLimit && problem && !problem.permanent; attempt += 1) {
    await promptEnter(`${label}${problem.message}\n请先确认当前Chrome已经能正常显示商品列表，再点击运营台“继续执行”（第 ${attempt}/${retryLimit} 次）：`, config);
  ...
  try {
    ...
    for (const [jobIndex, job] of config.jobs.entries()) {
      ...
      if (!config.browser.headless && config.browser.pauseBeforeStart) {
        await promptEnter(`商品池刷新已暂停。请确认当前Chrome显示 ${config.siteCountry} / ${config.language} / ${config.currency} 的“${job.subcategory}”商品列表；完成登录或验证后回到本窗口按 Enter，程序将自动确认 ${job.sortOrder} 排序：`, config);
      }
      await handleChallenge(page, config, '商品池刷新');
      if (await isGoneListingPage(page) || !await hasProductLinks(page, config)) {
...
export async function captureCurrentCatalog(config, db) {
  const { runCatalogCaptureCommand } = await import('./app/commands/catalog-capture.mjs');
  return runCatalogCaptureCommand(config, { targetCount: config.catalog?.targetCount ?? config.targetCount });
...
export async function crawl(config, db) {
  ...
  try {
    ...
    for (const [jobIndex, job] of config.jobs.entries()) {
      ...
      if (!config.browser.headless && config.browser.pauseBeforeStart) {
        await promptEnter(`请确认当前页面已设置为 ${config.siteCountry} / ${config.language} / ${config.currency} / ${job.sortOrder}，确认后按 Enter：`, config);
      }
      await sleep(500);
      await handleChallenge(page, config, '分类页');
      if (new URL(page.url()).pathname !== expectedPathname) {
    ...
    for (const [index, product] of candidates.entries()) {
      ...
      try {
        ...
        if (!operatorConfirmed && !config.browser.headless && config.browser.pauseBeforeStart) {
          await promptEnter('评论抓取已暂停。请在当前Chrome中完成登录、验证码或安全验证，并确认商品详情和评价区域正常显示；完成后回到本窗口按 Enter 开始：', config);
