...
export function createSourcingController({service,repository,settingsStore,poolBatches=null,pathDialog,validateWorkbook=validateExistingWorkbook,validatePaths=validateConfiguredPaths,runIdFactory=()=>crypto.randomUUID()}={}) {
  ...
  async function startImport({scanToken:providedToken,runId=null,poolId,batchName}={}) {
    ...
    if(poolBatches&&(!scannedPool||scannedPool.id!==poolId))throw coded('SCAN_STALE','商品池已改变，请重新扫描。');
    const previousCurrentRunId=currentRunId;
    busy=true;state='IMPORTING';currentRunId=runId??runIdFactory();
    try {
      const result=await service.startImport({scanToken,runId:currentRunId,onStructured:scannedPool?({runId})=>poolBatches.save(runId,scannedPool,batchName):null});
