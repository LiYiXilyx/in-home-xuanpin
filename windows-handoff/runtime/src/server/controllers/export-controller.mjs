...
import {exportCatalogImages} from './catalog-images.mjs';
...
export function createExportController({ config,repository,service,openTarget=defaultOpenTarget,exportWorkbook=exportOperationsWorkbook,
  ...
  const outputDir=config.export.outputDir;let imageDestination=null,lastImageFolder=null;
  ...
  return {
    ...
    async listImageExports(){const dir=path.join(outputDir,'catalog-scoped');const names=await fsp.readdir(dir).catch(e=>{if(e.code==='ENOENT')return [];throw e;});const files=[];for(const name of names.filter(n=>n.startsWith('catalog-')&&n.endsWith('.xlsx.images.json'))){const file=name.slice(0,-12);try{const stat=await fsp.stat(path.join(dir,file));const rows=JSON.parse(await fsp.readFile(path.join(dir,name),'utf8'));files.push({file_name:file,product_count:rows.length,modified_at:stat.mtime.toISOString()});}catch{}}return files.sort((a,b)=>b.modified_at.localeCompare(a.modified_at));},
    async chooseImageFolder(){const result=await chooseNativePath({kind:'IMAGE_DOWNLOAD_DIRECTORY',currentPath:imageDestination||path.join(outputDir,'catalog-scoped')});if(!result.cancelled)imageDestination=result.path;return result;},
    async openImageFolder(){if(!lastImageFolder)throw Error('请先完成一次图片下载');await openTarget(lastImageFolder,{kind:'folder'});return {folder:lastImageFolder};},
    async exportCatalogImages(body){const result=await exportCatalogImages(path.join(outputDir,'catalog-scoped'),body.file_name,imageDestination);lastImageFolder=result.folder;return result;},
    async export() {
    ...
    async openCatalogFolder() { const target=path.join(outputDir,'catalog-scoped');await fsp.access(target);await openTarget(target,{kind:'folder'});return {message:'采集导出目录已打开。'}; },
...
export function defaultOpenTarget(target,{ kind='folder',platform=process.platform,spawnImpl=spawn }={}) {
  if(platform==='darwin')return new Promise((resolve,reject)=>{
    const child=spawnImpl('/usr/bin/open',[target],{stdio:'ignore',shell:false});
    child.once('error',reject);child.once('exit',code=>code===0?resolve():reject(new AppError(`Mac 打开失败，退出码 ${code ?? '未知'}。`,{code:'OPEN_TARGET_FAILED'})));
  });
  return new Promise((resolve,reject) => {
    const script=[
      ...
      '} else { $explorer = Join-Path $env:WINDIR "explorer.exe"; $folderArgument = [char]34 + $target + [char]34; Start-Process -FilePath $explorer -ArgumentList $folderArgument -WindowStyle Normal -ErrorAction Stop }'
    ].join('; ');
    const child=spawnImpl('powershell.exe',['-NoLogo','-NoProfile','-NonInteractive','-WindowStyle','Hidden','-Command',script],{
