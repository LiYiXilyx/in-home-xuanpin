...
export async function createOperationsServer(options={}) {
  ...
  const catalogService=createCatalogCampaignService(db,{claimInspectionService:catalogClaimInspectionService,activityRegistry:catalogActivityRegistry});
  ...
  const sourcingRepository=createSourcingRepository(sourcingDb);const poolBatches=createPoolBatches(db,sourcingDb);
  ...
  if(!sourcingReviewController) {
    const defaultReviewRunId=options.sourcingReviewRunId??process.env.SOURCING_REVIEW_RUN_ID??'yingdao_random5_v1_20260831_001';
    const runControllers=new Map();
    const buildRunController=async reviewRunId=>{
    if(!sourcingRepository.getImport(reviewRunId))throw Error('复核批次不存在');
    ...
    return createSourcingReviewController({service:sourcingReviewService,imageResolver});
    };
  ...
  const trackingService=createTrackingService({sourceDb:db,databasePath:path.join(path.dirname(config.app.databasePath),'period-tracking.db')});
  const trackingTimer=setInterval(()=>{try{trackingService.sync();}catch(e){console.error('周期跟踪归档失败',e.message);}},60000);trackingTimer.unref();
  const serveStatic=createStaticServer(path.join(projectDir,'ui'));
  const router=createRouter({ trackingService,statusService,browserController,jobController,reviewController,reviewQueueController,catalogController,exportController,testController,sourcingController,sourcingReviewController,temuMarketEvidenceController,serveStatic,
  ...
  return {
    ...
    async close() {
      ...
      clearInterval(trackingTimer);trackingService.close();
      db.close();
      sourcingDb.close();
      temuContextDb?.close();
    }
