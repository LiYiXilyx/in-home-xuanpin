...
export function createCatalogCampaignRepository(db,{ now=() => new Date().toISOString() }={}) {
  ...
  function upsertStaging(campaign,source,batchId,product,screeningStatus) {
    ...
    ON CONFLICT(campaign_id,platform,goods_id) DO UPDATE SET
      ...
      image_url=CASE WHEN excluded.image_url IS NOT NULL THEN excluded.image_url WHEN catalog_staging_products.image_url LIKE 'data:%' OR catalog_staging_products.image_url LIKE '%/ad_tag/%' OR catalog_staging_products.image_url LIKE '%/badge/%' OR catalog_staging_products.image_url LIKE '%/icon/%' THEN NULL ELSE catalog_staging_products.image_url END,
