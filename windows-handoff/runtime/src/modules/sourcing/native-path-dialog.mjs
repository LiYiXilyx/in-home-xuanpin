...
const KINDS=new Set(['RAW_DIRECTORY','IMAGE_CACHE_DIRECTORY','IMAGE_DOWNLOAD_DIRECTORY','ANALYSIS_WORKBOOK']);
...
const MAC_SCRIPTS=Object.freeze({
  IMAGE_DOWNLOAD_DIRECTORY:'POSIX path of (choose folder with prompt "选择商品图片下载目录")',
...
export async function chooseNativePath({platform=process.platform,kind,currentPath=null,runProcess=defaultRunProcess}={}) {
  ...
  if(platform==='darwin') call={command:'osascript',args:['-e',MAC_SCRIPTS[kind]],shell:false};
    ...
    args:['-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-Sta','-File',SCRIPT_PATH,'-Kind',kind,...(currentPath?['-CurrentPath',String(currentPath)]:[])],
    shell:false,
  };
  ...
  try {
    ...
    if(result?.code && result.code!==0) throw coded('PATH_DIALOG_FAILED',/Execution_Policies|UnauthorizedAccess|SecurityError/i.test(result.stderr||'')?'Windows 阻止了目录选择脚本，请联系维护人员检查启动策略。':(result.stderr||`native dialog exited ${result.code}`));
