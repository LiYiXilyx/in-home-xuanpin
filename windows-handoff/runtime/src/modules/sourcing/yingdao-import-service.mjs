...
export function createYingdaoImportService({
  ...
  async function scan(config) {
    ...
    assertPoolGoods(preview,config.allowedGoodsIds);
    ...
    scans.set(scanToken,{
      config:normalizedConfig,allowedGoodsIds:config.allowedGoodsIds,
      status,
      sourceManifestSha256:preview.sourceManifestSha256,
    });
  ...
  async function startImport({ scanToken,runId=null,onStructured=null,onImages=null }={}) {
    ...
    assertPoolGoods(current,prior.allowedGoodsIds);
...
function assertPoolGoods(result,allowed){if(!allowed)return;const ids=new Set(allowed.map(String));const mismatched=[...new Set([...(result.files??[]),...(result.candidates??[])].map(c=>String(c.temu_goods_id)).filter(id=>!ids.has(id)))];if(mismatched.length)throw codedError('SOURCING_POOL_MISMATCH','有 '+mismatched.length+' 个商品不属于所选商品池，请换商品池或将不同类目的 Excel 分文件夹导入。商品 ID：'+mismatched.slice(0,10).join('、'));}
