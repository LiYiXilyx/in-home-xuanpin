import {createRecapturePoolMerge} from './recapture-pool-merge.mjs';
import {createManualRecapture} from './manual-recapture.mjs';
...
import { screenCatalogElectronicRisk, screenManualCatalogElectricalRisk } from './electronic-screening.mjs';
...
function assertBindingAfterPause(campaign,binding){
  const fence=campaign.config?.operatorBindingInvalidatedAt;
  if(fence&&!(Date.parse(binding?.bound_at)>Date.parse(fence)))throw new AppError('旧绑定已失效，请重新检测并绑定当前页面。',{code:'CATALOG_BINDING_INVALIDATED'});
}
...
export function createCatalogCampaignService(db,{ now=() => new Date().toISOString(),screenElectronicRisk=screenCatalogElectronicRisk,
  ...
  const operatorEntry=createOperatorEntryService({db,repository,initialRepository,now,activityRegistry,claimBlockers:()=>claimBlockers()});
  ...
  function createCampaignRecord({ id=null,name,campaignType='expansion',profile,baselinePoolCount=0,targetCount=null,browserContext=null,
    ...
    if (!configExtras.trackingRecapture && (campaignType==='refresh' || campaignType==='expansion')) {
  ...
  function captureBatch({ campaignId,sourceId,batchId,pageUrl=null,pageTitle=null,capturedAt=now(),cards=[],
    ...
    const campaign=requireCampaign(campaignId);
    assertBindingAfterPause(campaign,pageBinding);
    ...
    return transaction(db,() => {
      ...
      for (const raw of cards) {
        ...
        const platform='temu';
        const screening=captureMode===MANUAL_PASSIVE_CAPTURE_MODE?screenManualCatalogElectricalRisk(raw):captureProfile.profile_kind==='CAPTURE_ONLY'?captureOnlyScreening():screenElectronicRisk(raw);
        ...
        if (!campaign.config?.trackingRecapture && (campaign.campaignType==='refresh' || campaign.campaignType==='expansion') && screening.decision==='passed'
  ...
  function captureExtensionBatchImpl(input) {
    ...
    assertBindingAfterPause(requireCampaign(campaignId),input.page_binding);
  ...
  const poolMerge=createRecapturePoolMerge({db,repository,now,activityRegistry});
  const manualRecapture=createManualRecapture({db,repository,now,activityRegistry,createCampaignRecord});
  const api={previewRecaptureMerge:poolMerge.preview,mergeRecapturePool:poolMerge.merge,listManualTasks:manualRecapture.list,switchManualTask:manualRecapture.switchTask,pauseManualTask:manualRecapture.pauseCurrent, pauseOperatorInitial:operatorEntry.pauseInitial,continueOperatorInitial:operatorEntry.continueInitial,resolveOperatorEntry:operatorEntry.resolve,createCampaign,describeOperatorProfile,createOperatorManualCampaign,createOperatorInitialCampaign,currentOperatorManualContext,transitionCampaign,updateBrowserContext,createSource,captureBatch,submitQa,failCampaign,
    withOperatorExport(campaignId,fn){const queues=repository.listRpaQueues(campaignId);if(!activityRegistry||queues.length!==1)return fn();return activityRegistry.run({campaignId,queueId:queues[0].id},'excel_export',fn);},
  ...
  function rpaContext(queue,{ exposeClaimToken=true }={}) {
    ...
    return { queue:{ ...queue,claimToken:exposeClaimToken ? queue.claimToken:undefined },
      campaign:{ id:campaign.id,name:campaign.name,status:campaign.status,categoryKey:campaign.categoryKey,
        ...
        captureTransportPolicy:resolveCaptureTransportPolicy({campaign,profile}).policy,
        categoryProfileVersion:campaign.categoryProfileVersion,targetGate:campaign.targetGate,targetCount:campaign.targetCount,
        quantityMode:quantityPolicy.quantityMode,captureLimit:quantityPolicy.captureLimit,
        remaining:quantityPolicy.remaining,targetReached:quantityPolicy.targetReached,
        rawObservedCount:campaign.rawObservedCount,electronicExcludedCount:campaign.electronicExcludedCount,
...
function validateExtensionCard(value,index) {
  ...
  if(value.currency!=null&&!['EUR'].includes(value.currency))throw new AppError('商品币种与德国站不符。',{code:'CATALOG_CONTEXT_MISMATCH'});
  const result={ currency:value.currency??null,goods_id:goodsId,href:optionalString(value.href,'href',2048),title:optionalString(value.title,'title',1000),
