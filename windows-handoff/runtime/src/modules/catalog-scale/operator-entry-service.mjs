...
export function createOperatorEntryService({db,repository,initialRepository,now,claimBlockers=()=>({}),activityRegistry=null}) {
 ...
 function pauseInitial({profile,campaignId,queueId,expectedClaimGeneration,requestId}){
  if(typeof requestId!=='string'||!requestId.trim())fail('OPERATOR_REQUEST_ID_REQUIRED');
  if(!Number.isInteger(expectedClaimGeneration)||expectedClaimGeneration<1)fail('OPERATOR_MANUAL_CONTEXT_MISMATCH');
  return transaction(db,()=>{
   const c=repository.getCampaign(campaignId);validateScope(c,validateCategoryProfile(profile));
   if(c.campaignType!=='initial')fail('INITIAL_CAMPAIGN_REQUIRED');
   const payload={campaignId,queueId,expectedClaimGeneration,categoryKey:profile.category_key,categoryProfileVersion:profile.category_profile_version};
   const prior=c.config.operatorPause??[],replay=prior.find(r=>r.requestId===requestId);
   if(replay){
    if(JSON.stringify(replay.payload)!==JSON.stringify(payload))fail('OPERATOR_CREATE_IDEMPOTENCY_CONFLICT');
    const q=repository.getRpaQueue(queueId);
    if(c.status!=='paused'||q?.status!=='pending'||q.claimToken)fail('OPERATOR_MANUAL_CONTEXT_MISMATCH');
    return {...replay.result,idempotentReplay:true};
   }
   if(!['running','manual_required'].includes(c.status))fail('CAMPAIGN_NOT_ACTIVE');
   const {q,source,runs}=readChildren(c);
   const claim=db.prepare('SELECT claim_token,claim_generation FROM catalog_rpa_queue WHERE id=?').get(q.id);
   if(q.id!==queueId||claim.claim_generation!==expectedClaimGeneration||!claim.claim_token||runs.length!==1)fail('OPERATOR_MANUAL_CONTEXT_MISMATCH');
   if(!activityRegistry)fail('CATALOG_TASK_BUSY','无法核验任务活动，拒绝暂停。');
   const activity=activityRegistry.snapshot({campaignId,queueId});
   if(Object.entries(activity).some(([key,value])=>key!=='liveBinding'&&value))fail('CATALOG_TASK_BUSY','当前任务正在处理数据，请等待完成后再暂停。');
   const at=now(),checkpoint={...q.checkpoint,runner_state:'UNBOUND',capture_paused:true,last_action:'operator_pause_for_switch'};
   for(const key of Object.keys(checkpoint))if(key==='status'||/binding|fingerprint|bound_|page_context/i.test(key))delete checkpoint[key];
   repository.transitionCampaign(campaignId,'paused');repository.transitionSource(source.id,'pending');
   repository.transitionRpaQueue(q.id,'pending',{checkpoint,clearError:true});
   db.prepare('UPDATE catalog_rpa_queue SET claim_token=NULL,claimed_at=NULL,heartbeat_at=NULL WHERE id=?').run(q.id);
   db.prepare('UPDATE catalog_source_runs SET finished_at=?,stop_reason=? WHERE id=? AND finished_at IS NULL').run(at,'OPERATOR_PAUSED_FOR_CATEGORY_SWITCH',runs[0].id);
   const result={campaign_id:campaignId,status:'paused',queue_id:q.id,source_id:source.id,binding_status:'UNBOUND',idempotentReplay:false};
   const config={...c.config,operatorBindingInvalidatedAt:at,operatorPause:[...prior,{requestId,payload,result,at,previous:{campaign:c.status,queue:q.status,source:source.status},activity}]};
   db.prepare('UPDATE catalog_campaigns SET config_json=? WHERE id=?').run(JSON.stringify(config),campaignId);
   return result;
  });
 }
 return {resolve,continueInitial,pauseInitial,assertNoForeignClaim};
}
