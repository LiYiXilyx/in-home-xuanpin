export function createYingdaoApi({fetchImpl=globalThis.fetch}={}){if(typeof fetchImpl!=='function')throw new Error('YingDao API缺少 fetch。');
  ...
  return{
    ...
    pools:()=>request('/api/sourcing/pools'),scan:poolId=>request('/api/sourcing/scan',{method:'POST',body:{poolId}}),startImport:(scanToken,poolId,batchName)=>request('/api/sourcing/imports',{method:'POST',body:{scanToken,poolId,batchName}}),
