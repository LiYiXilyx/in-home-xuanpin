const ERROR_MESSAGES=Object.freeze({
  CATALOG_RPA_CLAIM_CONFLICT:'已有采集任务占用队列，当前操作尚未生效。\n① 向下查看“当前采集任务”。如果是首次采集，点击“暂停当前首次任务，切换类目”；如果是复采任务，到下方“已有商品复采”点击“暂停当前任务，释放采集占用”。已采商品和进度会保留。\n② 如果“自动截图队列”或差评采集正在运行，先到对应页面暂停，等待显示已暂停或已停止。\n③ 返回本页，重新选择需要的类目，点击“开始首次采集”或“继续首次采集”。确认当前任务变成目标类目后，再到 Temu 插件重新检测、绑定、采集。\n如果仍报错，请在“日志系统”导出日志，连同报错截图交给维护人员。此提示会保留到暂停成功或目标任务创建／继续成功。',
...
export function buildOperatorCategoryDraft({displayName,pageCategoryName,aliases,parentCategory,breadcrumbs,listingUrl}={}){
  ...
  return{display_name:String(displayName??'').trim(),page_category_name:String(pageCategoryName??'').trim(),
    category_aliases:lines(aliases),parent_category:String(parentCategory??'').trim()||lines(breadcrumbs).at(-2)||lines(breadcrumbs)[0]||'',breadcrumbs:lines(breadcrumbs),
