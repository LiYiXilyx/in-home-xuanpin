...
test('unclaimed context response still mounts read-only recognition after operator pause',async()=>{
 let mounted=0;const requests=[];const box=vm.createContext({document:{},chrome:{runtime:{sendMessage(m,cb){requests.push(m.type);cb({ok:false,error:{code:'CATALOG_RPA_NOT_CLAIMED',message:'没有已领取的Catalog RPA来源。'}});}}},TemuCatalogManualBinding:{},TemuCatalogOverlayMode:{resolveCatalogOverlayMode:c=>c?'MANUAL_BIND':'NO_CONTEXT'},TemuCatalogOperatorOverlay:{mount(){mounted++;}}});
 vm.runInContext(fs.readFileSync(new URL('../../browser-extension/catalog-manual-passive-runner.js',import.meta.url),'utf8'),box);
 await new Promise(r=>setImmediate(r));assert.equal(mounted,1);assert.ok(box.TemuCatalogManualPassiveRunner);assert.equal(box.TemuCatalogManualPassiveRunner.binding,null);
 assert.deepEqual(requests,['GET_CATALOG_CURRENT']);
});
