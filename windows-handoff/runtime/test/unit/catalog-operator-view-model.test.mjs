...
test('DOM optional transport is shown separately from page identity and partial Network is a warning',()=>{
  const value=girls({networkDiagnostics:{dom_unique_goods:40,network_unique_goods:1,network_enriched_goods:1,capture_transport_policy:'DOM_REQUIRED_NETWORK_OPTIONAL',capture_transport_status:'READY_WITH_DOM'},detection:{health:{status:'READY',checks:{country:true,language:true,currency:true,category:true,products:true,sort:true,evidenceReady:true},warnings:['NETWORK_COVERAGE_PARTIAL']},observed:{cardCount:40,domReady:true,networkReady:true,captureTransportPolicy:'DOM_REQUIRED_NETWORK_OPTIONAL',captureTransportStatus:'READY_WITH_DOM'}}});
  const model=load().build(value),transport=model.health.rows.find(row=>row.key==='captureTransport');assert.equal(transport.actual,'READY_WITH_DOM');assert.equal(transport.passed,true);assert.equal(model.technical.captureTransportPolicy,'DOM_REQUIRED_NETWORK_OPTIONAL');assert.deepEqual(Array.from(model.health.warnings),['NETWORK_COVERAGE_PARTIAL']);
});
