...
const transportPolicySource=fs.readFileSync(path.join(root,'browser-extension/catalog-capture-transport-policy.js'),'utf8');
function loadModule(){const sandbox=vm.createContext({console,URL,Date,document:{body:{innerText:''},documentElement:{},getElementById:()=>null},location:{href:'https://www.temu.com/de-en/category-b.html',pathname:'/de-en/category-b.html'}});vm.runInContext(breadcrumbSource,sandbox);vm.runInContext(bindingSource,sandbox);vm.runInContext(transportPolicySource,sandbox);vm.runInContext(runnerSource,sandbox);return sandbox.TemuCatalogManualPassiveRunnerModule;}
...
function openEndedContext(){const value=context({accepted:1000,checkpoint:{quantity_mode:'OPEN_ENDED'}});value.campaign={...value.campaign,campaignType:'initial',quantityMode:'OPEN_ENDED',targetCount:null,captureLimit:null,remaining:null,targetReached:null,captureTransportPolicy:'DOM_REQUIRED_NETWORK_OPTIONAL'};value.profile={...value.profile,profile_kind:'CAPTURE_ONLY',listing_url:'https://www.temu.com/de-en/category-b.html',breadcrumbs:['Home','Category B']};return value;}
function healthy(overrides={}){return{url:'https://www.temu.com/de-en/category-b.html?sort=top',siteCountry:'DE',language:'en',currency:'EUR',category:'Category B',categoryCandidates:[{value:'Category B',source:'BREADCRUMB_TERMINAL'}],breadcrumbs:['Home','Category B'],sortOrder:'Top Sales',cardCount:2,goodsIds:new Set(['1','2']),domReady:true,searchNoResults:false,captchaBlocking:false,...overrides};}
function harness(initial){let current=structuredClone(initial),page=healthy(),network={network_cache_size:2,network_enriched_goods:2},submits=0,resumes=0,lastSubmit=null;const writes=[];return{dependencies:{getContext:async()=>structuredClone(current),scan:()=>page,networkDiagnostics:()=>network,passiveCandidates:({transportPolicy}={})=>transportPolicy?.policy==='NETWORK_ENRICHED_REQUIRED'&&Number(network.network_enriched_goods)===1?[{goods_id:'1'}]:[{goods_id:'1'},{goods_id:'2'}],submitPassive:async options=>{submits+=1;lastSubmit=options;return{batch:{batchId:options.batchId},audit:{acceptedGoods:options.goodsIds.length,campaignStagingDeduped:0}};},checkpoint:async payload=>{writes.push(payload);current.queue.checkpoint={...current.queue.checkpoint,...payload.checkpoint};return current.queue;},resume:async payload=>{resumes+=1;current.campaign.status='running';current.queue.status='opening';current.source.status='opening';current.queue.checkpoint={...payload.checkpoint};return current.queue;},now:()=> '2026-08-31T00:00:00.000Z'},setPage:value=>{page=value;},setNetwork:value=>{network=value;},setContext:value=>{current=structuredClone(value);},get submits(){return submits;},get resumes(){return resumes;},get lastSubmit(){return lastSubmit;},get writes(){return writes;}};}
...
test('strict transport becomes ready with a non-empty intersection and submits only that intersection',async()=>{
  ...
  const result=await runner.detectCurrentPage();assert.equal(result.state,STATES.PAGE_READY);assert.equal(result.detection.observed.captureTransportStatus,'READY_NETWORK_ENRICHED');assert.equal(result.networkDiagnostics.network_enriched_goods,1);await runner.bindCurrentPage();await runner.captureCurrentPage();assert.deepEqual(h.lastSubmit.goodsIds,['1']);assert.equal(h.submits,1);
});

test('CAPTURE_ONLY Initial treats valid DOM as ready when Network coverage is partial',async()=>{
  const {ManualPassiveRunner,STATES}=loadModule(),initial=openEndedContext(),h=harness(initial),runner=new ManualPassiveRunner(h.dependencies,initial);await runner.restore(initial);h.setNetwork({network_cache_size:1,network_unique_goods:1,dom_unique_goods:2,network_enriched_goods:1});
  const detected=await runner.detectCurrentPage();assert.equal(detected.state,STATES.PAGE_READY);assert.equal(detected.detection.observed.captureTransportStatus,'READY_WITH_DOM');assert.ok(detected.detection.health.warnings.includes('NETWORK_COVERAGE_PARTIAL'));
  await runner.bindCurrentPage();await runner.captureCurrentPage();assert.deepEqual(h.lastSubmit.goodsIds,['1','2']);assert.equal(h.lastSubmit.transportPolicy.policy,'DOM_REQUIRED_NETWORK_OPTIONAL');
});
