...
test('pool scope rejects foreign goods before writing any import',async t=>{
 const sourceDir=await setupSource(t),repository=memoryRepository();
 const service=createYingdaoImportService({repository,loadWorkbook:async()=>({headers:HEADERS,rows:workbookRows()}),workbookStage:NOOP_WORKBOOK_STAGE});
 await assert.rejects(()=>service.scan({...config(sourceDir),allowedGoodsIds:['999']}),e=>e.code==='SOURCING_POOL_MISMATCH');
 assert.equal(repository.writes,0);
});
