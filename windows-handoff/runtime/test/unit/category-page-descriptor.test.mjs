...
test('selected sort line survives dropdown option text without accepting an unselected Top sales option',async()=>{
 const api=await load();
 const n=innerText=>({innerText,getAttribute:()=>null,getClientRects:()=>[{}],closest:()=>null});
 const control=n('Sort by: Top sales\nRelevance\nTop sales\nPrice low to high');
 const doc={documentElement:{lang:'en'},querySelectorAll(s){
  if(s==='[aria-label*="breadcrumb" i] li')return ['Home','Automotive','Covers'].map(n);
  if(s==='button,[role="button"],[aria-selected="true"]')return [control];
  if(s==='a[href*="-g-"],a[href*="goods_id="]')return [Object.assign(n('€10'),{href:'https://www.temu.com/seat-g-6011.html'})];
  return [];
 }};
 assert.equal(api.parseCategoryPage(doc,base).sort_order,'Top Sales');
 control.innerText='Sort by: Relevance\nTop sales';
 assert.throws(()=>api.parseCategoryPage(doc,base),e=>e.code==='CATEGORY_SORT_MISMATCH');
 control.innerText='Sort by: Price low to high\nTop sales';
 assert.throws(()=>api.parseCategoryPage(doc,base),e=>e.code==='CATEGORY_SORT_MISMATCH');
});
test('currency in product card sibling is recognized without accepting unrelated page prices',async()=>{
 const api=await load();
 const n=t=>({innerText:t,getAttribute:()=>null,getClientRects:()=>[{}],closest:()=>null});
 const link=Object.assign(n('Item picture'),{href:'https://www.temu.com/seat-g-6011.html'});
 const card=Object.assign(n('Seat €11.79'),{querySelectorAll:()=>[link],parentElement:null});link.parentElement=card;
 const doc={documentElement:{lang:'en'},querySelectorAll(s){
 if(s==='a[href*="-g-"],a[href*="goods_id="]')return[link];
 if(s==='[aria-label*="breadcrumb" i] li')return['Home','Automotive','Covers'].map(n);
 if(s==='button,[role="button"],[aria-selected="true"]')return[n('Sort by: Top sales')];return[];
 }};
 const url='https://www.temu.com/de-en/covers-o3-1198.html';
 assert.equal(api.parseCategoryPage(doc,url).currency,'EUR');
 card.innerText='Seat $11.79';assert.throws(()=>api.parseCategoryPage(doc,url),e=>e.code==='CATEGORY_MARKET_MISMATCH');
 card.innerText='Seat';card.parentElement=Object.assign(n('Cart €20'),{querySelectorAll:()=>[link,{href:'https://www.temu.com/other-g-999.html'}]});
 assert.throws(()=>api.parseCategoryPage(doc,url),e=>e.code==='CATEGORY_MARKET_MISMATCH');
});
