...
test('page removing overlay host reattaches the same panel without actions; destroy disconnects recovery',()=>{
 const api=modules();let notify,disconnected=false,host=null,subscriptions=0;
 api.clearTimeout=()=>{};
 api.MutationObserver=class{constructor(fn){notify=fn;}observe(){}disconnect(){disconnected=true;}};
 const doc={getElementById:id=>host?.isConnected&&host.id===id?host:null,createElement:()=>({dataset:{},querySelector:()=>null,isConnected:false,remove(){this.isConnected=false;}}),documentElement:{append(node){host=node;node.isConnected=true;}}};
 const runner={snapshot:()=>snapshot(),subscribe(fn){subscriptions++;fn(snapshot());return()=>subscriptions--;}};
 const panel=api.TemuCatalogOperatorOverlay.mount({runner,document:doc});const original=host;
 host.remove();notify?.();assert.equal(host.isConnected,true);assert.equal(host,original);assert.equal(subscriptions,1);
 panel.destroy();assert.equal(disconnected,true);assert.equal(host.isConnected,false);assert.equal(subscriptions,0);
});
