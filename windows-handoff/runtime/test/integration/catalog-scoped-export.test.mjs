...
test('preview requires exact current revision and formal export requires exact active Pool tuple with zero writes',async t=>{
  ...
  const sharp=(await import('sharp')).default;const jpg=await sharp({create:{width:16,height:16,channels:3,background:'#ffffff'}}).jpeg().toBuffer();
  const exporter=createCatalogScopedExportService({repository,outputDir:path.join(f.directory,'exports'),imageLoader:async()=> 'data:image/jpeg;base64,'+jpg.toString('base64')});
  ...
  assert.equal(formalFile.sheet_names.length,6);
  const {fingerprintSheet05}=await import('../../src/modules/sourcing/random5-workbook.mjs');await fingerprintSheet05(formalFile.saved_path);
  const {loadRunOpportunityWorkbook}=await import('../../src/modules/sourcing/review-opportunity-workbook.mjs');await loadRunOpportunityWorkbook({workbookPath:formalFile.saved_path,runGoodsIds:['20','3']});
