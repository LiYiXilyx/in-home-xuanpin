...
test('capture-only Manual Bind excludes electrical goods and preserves mechanical goods',async t=>{
  ...
  const value=payload(f,created.campaignId,source.id,'batch-1',[card('1','USB rechargeable bluetooth pet camera'),card('2','12V blade fuses'),card('3','Pet bowl')]);
  const result=f.service.captureExtensionBatch(value);assert.equal(result.batch.stagingCount,1);assert.equal(result.batch.excludedCount,2);assert.equal(result.audit.electronicExcluded,2);
