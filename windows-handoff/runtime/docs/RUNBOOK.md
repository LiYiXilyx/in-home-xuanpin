...
如果运营台提示“请人工点击 Try again”，请切换到采集 Chrome，滚动到商品列表底部，人工点击 `Try again`；确认页面已经出现新商品后，再回运营台点击“继续”。不要在商品没有变化时连续反复点击。
...
需要验证“空库采集 → 导出 → 数量对账”时，不要清除正式运营台的数据。双击根目录的 `启动Temu测试运营台.vbs`，系统会在 `http://127.0.0.1:37822` 打开带红色“测试模式”标志的独立运营台。正式运营台仍使用 37821 端口，两者互不替代。

测试模式只使用以下位置：

- 数据库：`data/test/temu_research_test.db`
- Excel 和图片：`outputs/test-run/`
- 重置前备份：`backups/test/`
- 日志：`logs/test-dashboard.log`

测试流程：

1. 确认页面顶部显示红色“测试模式”，标题为“Temu 测试运营台”。没有红色标志时禁止执行测试重置。
2. 按 External Chrome 流程准备 Temu 页面，点击“连接已有 Chrome”和“验证当前页面”，确认状态为 `READY`。
3. 如需从零开始，先安全暂停或取消正在运行的测试任务，再点击红色“重置测试数据”。
4. 第一次确认窗口选择“确定”，第二次输入 `重置测试数据`。两次确认缺一不可。
5. 重置完成后会自动生成 0 商品空白 Excel；可立即点击“打开 Excel”确认主表为空。
6. 再执行 100 条测试，导出并打开测试 Excel，对比数据库、页面和 Excel 数量。

“重置测试数据”会先备份测试数据库和整个测试输出目录，再清空测试商品、任务、快照、图片记录和质量记录；不会访问或修改 `data/temu_research_v2.db`、`outputs/week1-mvp/` 或正式 300 商品池。测试模式不能用来清除正式数据。
