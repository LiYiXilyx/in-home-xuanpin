...
(() => {
  ...
  function error(code,message) { const detail=message && typeof message==='object'?message:null;const value=new Error(typeof message==='string'?message:typeof detail?.message==='string'?detail.message:'本地运营台返回未知错误。');value.code=detail?.code??code;return value; }
  ...
  async function capture({ campaignId,sourceId,batchId=globalThis.crypto?.randomUUID?.() ?? `catalog-${Date.now()}`,cards=null,captureMode=null,pageBinding=null }) {
    ...
    for (let index=0;index<chunks.length;index+=1) {
      ...
      if(MANUAL_CAPTURE_MODES.has(captureMode))payload.batch_id=await manualPayloadBatchId(payload);
  ...
  async function manualPayloadBatchId(payload) {
    const {batch_id,captured_at,...content}=payload;
    const bytes=new TextEncoder().encode(JSON.stringify(content));
    const digest=await globalThis.crypto.subtle.digest('SHA-256',bytes);
    return 'manual-v2-'+[...new Uint8Array(digest)].map(x=>x.toString(16).padStart(2,'0')).join('');
  }
