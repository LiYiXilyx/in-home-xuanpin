...
(() => {
  ...
  function parseElement(element,{ baseUrl,sequence }) {
    ...
    const image=images.filter(i=>usableImage(i,baseUrl)).sort((a,b)=>{
      const area=i=>{const r=i.getBoundingClientRect?.();return Math.max(r?.width||0,i.naturalWidth||0)*Math.max(r?.height||0,i.naturalHeight||0);};
      return area(b)-area(a);
    })[0];
    const title=cleanTitle(firstText(link.getAttribute('aria-label'),image?.getAttribute('alt'),link.innerText));
    ...
    return { goods_id:goodsId,href,title,image_url:usableImage(image,baseUrl),
      price_amount:prices[0] ?? null,currency:prices.length?'EUR':null,original_price_amount:prices[1] ?? null,sales_count:sales.parsed_sales_count,
  ...
  function parseHtmlFixture(html,{ baseUrl='https://www.temu.com/',startSequence=1 }={}) {
    ...
    while ((match=pattern.exec(html))) {
      ...
      const sales=parseSalesEvidence(rawText);
      cards.push({ goods_id:goodsId,href:absoluteUrl(href,baseUrl),title:cleanTitle(firstText(attribute(anchor[1],'aria-label'),attribute(imageTag,'alt'),decodeHtml(anchor[2].replace(/<[^>]+>/g,' ')))),
        image_url:absoluteUrl(attribute(imageTag,'src'),baseUrl),price_amount:prices[0] ?? null,currency:prices.length?'EUR':null,original_price_amount:prices[1] ?? null,
  ...
  function parseSalesEvidence(text) {
    const pattern=/([\d.,]+)\s*([KMB])?\+?\s*sold(?=\b|\d)/i;const match=String(text).match(pattern);
  ...
  function usableImage(image,baseUrl){
    if(!image)return null;
    const attr=name=>image.getAttribute?.(name);
    const sources=image.parentElement?.tagName==='PICTURE'?[...image.parentElement.querySelectorAll('source')]:[];
    const sets=[attr('srcset'),attr('data-srcset'),...sources.map(s=>s.getAttribute('srcset')||s.getAttribute('data-srcset'))];
    const setUrls=sets.flatMap(value=>String(value||'').split(',').map(part=>part.trim().split(/\s+/)[0]));
    for(const value of [image.currentSrc,attr('data-src'),attr('data-original'),attr('data-lazy-src'),...setUrls,attr('src')]){
      const url=absoluteUrl(value,baseUrl);
      if(url&&/^https?:/i.test(url)&&!/\/(?:ad_tag|badge|icon|placeholder|transparent)(?:\/|[_.-])/i.test(url))return url;
    }
    return null;
  }
  function cleanTitle(value) { return clean(value).replace(/^item picture\s+/i,'').replace(/\s*Open in new tab\.?$/i,'').replace(/^Top pick(?:\s+|(?=[A-Z0-9]))/,'').trim() || null; }
  ...
  globalThis.TemuCatalogParser=Object.freeze({ extractGoodsId,parseDocument,parseElement,parseHtmlFixture,parsePrices,cleanTitle,parseReviewCount,parseCompactCount,parseSalesEvidence,enrichCards });
})();
