...
function pendingCount(scan){
 if(!runner?.binding||!runner.context)return 0;
 const policy=globalThis.TemuCatalogCaptureTransportPolicy.resolveCaptureTransportPolicy({campaign:runner.context.campaign,profile:runner.context.profile});
 return mod.passiveCandidates({rawCards:scan.rawCards,submitted:runner.submitted,limit:300,transportPolicy:policy,pageBinding:runner.binding}).length;
}
document.addEventListener('click',async event=>{
 const button=event.target.closest?.('#hybrid-human-wait button[data-local-action]');if(!button)return;
 const panel=button.closest('#hybrid-human-wait');
 let feedback=panel.querySelector('[data-feedback]');if(!feedback){feedback=document.createElement('p');feedback.dataset.feedback='';panel.append(feedback);}
 button.disabled=true;feedback.textContent='正在发送…';
 try{
  const response=await chrome.runtime.sendMessage({type:'LOCAL_MOUSE_COMMAND',id:panel.dataset.jobId,action:button.dataset.localAction});
  if(!response?.ok)throw Error(response?.error?.message||'指令未接收');
  feedback.textContent=button.dataset.localAction==='resume'?'已收到，5秒后检查恢复，请保持商品页在前台':'已收到，正在结束';
 }catch(e){feedback.textContent='发送失败：'+e.message+'，可使用运营台按钮';}
