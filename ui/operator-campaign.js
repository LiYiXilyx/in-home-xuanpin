...
export function buildInitialCreatePayload({profile,campaignName,requestId}={}) {
  ...
  return {category_key:profile.category_key,category_profile_version:profile.category_profile_version,
    campaign_name:name,request_id:identity};
}
...
export function initialOperatorViewModel(current={}) {
  const qa=current.qa??{},status=qa.status??'NOT_RUN',currentCount=Number(current.current_unique??0);
  return {modeLabel:'不限数量 / OPEN_ENDED',currentCount,qaStatus:status,
    qaCandidateCount:Number(qa.qa_candidate_count??0),
...
function buildInitialActionPayload({campaignId,profile,requestId}={}) {
  const campaign=String(campaignId??'').trim(),identity=String(requestId??'').trim();
  if(!campaign||!identity||!profile?.category_key||!profile?.category_profile_version)throw new Error('Initial操作缺少明确 identity。');
  return {campaign_id:campaign,category_key:profile.category_key,
    category_profile_version:profile.category_profile_version,request_id:identity};
}
