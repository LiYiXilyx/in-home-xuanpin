...
export function mountYingdaoPanel({root,pollIntervalMs=3000,scheduler=globalThis,api=null}={}){
  ...
  function render(){const controls=deriveYingdaoControls(state);if(elements.state)elements.state.textContent=state.scanStatus;if(elements.error){elements.error.hidden=!state.error;elements.error.textContent=state.error?.message??'';}
    ...
    if(elements.scan)elements.scan.disabled=!controls.canScan;if(elements.import)elements.import.disabled=!controls.canImport;if(elements.retry)elements.retry.disabled=!controls.canRetry;
    setText(elements.runId,state.currentRun??'—');setText(elements.sourceFiles,state.progress.sourceFiles);setText(elements.goods,state.progress.validGoods);setText(elements.random5,state.random5.candidates);
  ...
  async function refresh(){if(!active)throw coded('YINGDAO_PANEL_NOT_MOUNTED','YingDao panel 已卸载。');if(refreshPromise)return refreshPromise;
    ...
    const operation=(async()=>{try{const [settings,current,review]=await Promise.all([client.settings(),client.currentImport(),client.reviewBootstrap?.()??null]);patchYingdaoState(state,applySourcingPayload(state,settings));patchYingdaoState(state,applySourcingPayload(state,current));
        if(review)patchYingdaoState(state,{currentRun:state.currentRun??review.run_id??null,reviewSummary:{awaiting:Number(review.awaiting_review??0),confirmed:Number(review.confirmed??0),noSelection:Number(review.no_selection??0),totalGoods:Number(review.total_goods??0),candidates:Number(current.random5_candidates??current.candidate_count??state.random5.candidates)}});return snapshotYingdaoState(state);}
