...
function renderMarketEvidence(reviewState){if(!evidence)return;const state=evidence.snapshot(),session=state.session,phases=state.evidence?.phases??[];evidenceFx=normalizeFxContext(reviewState.detail?.fx_context);const fxDisplay=marketEvidenceFxDisplay(evidenceFx);$('market-evidence-fx-rate').textContent=fxDisplay.rate;$('market-evidence-fx-source').textContent=fxDisplay.source;$('market-evidence-fx-as-of').textContent=fxDisplay.asOf;updateEvidenceRatio();$('market-evidence-query').value=state.query??'';$('market-evidence-session').textContent=session?`Session ${session.session_id} · ${session.status} · revision ${session.revision}`:'尚未创建 Session';$('market-evidence-before').textContent=`BEFORE：${phases.some(x=>x.phase==='BEFORE')?'已保存':'未保存'}`;$('market-evidence-after').textContent=`AFTER：${phases.some(x=>x.phase==='AFTER')?'已保存':'未保存'}`;$('market-evidence-create').disabled=!reviewState.currentGoodsId||Boolean(session&&session.status!=='CLOSED');$('market-evidence-copy-token').disabled=!state.bindToken;const root=$('market-evidence-results');root.replaceChildren();for(const phase of phases){const cards=JSON.parse(phase.cards_json??'[]'),shot=image(evidenceScreenshotUrl(state,phase.phase),`${phase.phase} 商品搜索结果安全区域截图`);shot.className='market-evidence-screenshot';root.append(text('strong',`${phase.phase} · ${cards.length} 个商品`),shot);for(const card of cards.slice(0,30)){const button=text('button',`${card.goods_id} · ${card.title??''} · ${money(card.price_eur,'EUR')}`);button.type='button';button.addEventListener('click',()=>{$('market-evidence-temu-price').value=card.price_eur??'';updateEvidenceRatio();});root.append(button);}}const writable=Boolean(session&&session.anchor_temu_goods_id===reviewState.currentGoodsId&&['AFTER_CAPTURED','ASSESSED'].includes(session.status));$('market-evidence-save').disabled=!writable;$('market-evidence-save-next').disabled=!writable;}
...
$('market-evidence-copy-query').addEventListener('click',()=>act(async()=>{await navigator.clipboard.writeText(selectedEvidenceQuery());$('reviewNotice').textContent='Temu 搜索词已复制，请由运营人员人工打开 Temu 并搜索。';}));
