...
let evidenceFx=normalizeFxContext(null);
...
function renderMarketEvidence(reviewState){
  ...
  evidenceFx=normalizeFxContext(reviewState.detail?.fx_context);const fxDisplay=marketEvidenceFxDisplay(evidenceFx);$('market-evidence-fx-rate').textContent=fxDisplay.rate;$('market-evidence-fx-source').textContent=fxDisplay.source;$('market-evidence-fx-as-of').textContent=fxDisplay.asOf;updateEvidenceRatio();
  for(const [slot,id] of [['1','market-evidence-query'],['2','market-evidence-query-2'],['3','market-evidence-query-3']]){const input=$(id),value=state.queryDrafts?.[slot]??'';if(!composingEvidenceInputs.has(id)&&input.value!==value)input.value=value;}$('market-evidence-query-slot').value=state.selectedQuerySlot??'1';
  const notice=$('market-evidence-notice');notice.hidden=!state.notice;$('market-evidence-notice-text').textContent=state.notice?`${state.notice.message}${state.notice.code?` (${state.notice.code})`:''}`:'';notice.dataset.kind=state.notice?.kind??'';
  ...
  const creating=state.action?.loading&&state.action.type==='CREATE_SESSION';$('market-evidence-create').disabled=!reviewState.currentGoodsId||creating||Boolean(session&&session.status!=='CLOSED');$('market-evidence-create').textContent=creating?'创建中…':'创建证据会话';$('market-evidence-copy-token').disabled=!state.bindToken;$('market-evidence-refresh').disabled=!session||state.action?.loading;$('market-evidence-continue').hidden=!(session&&session.status!=='CLOSED');
  ...
  const writable=Boolean(session&&session.anchor_temu_goods_id===reviewState.currentGoodsId&&['AFTER_CAPTURED','ASSESSED'].includes(session.status));$('market-evidence-save').disabled=!writable;$('market-evidence-save-next').disabled=!writable;
}
