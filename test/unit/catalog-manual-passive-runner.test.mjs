...
function harness(initial){let current=structuredClone(initial),page=healthy(),submits=0,resumes=0;const writes=[];return{dependencies:{getContext:async()=>structuredClone(current),scan:()=>page,networkDiagnostics:()=>({network_cache_size:2}),passiveCandidates:()=>[{goods_id:'1'},{goods_id:'2'}],submitPassive:async options=>{submits+=1;return{batch:{batchId:options.batchId},audit:{acceptedGoods:2,campaignStagingDeduped:0}};},checkpoint:async payload=>{writes.push(payload);current.queue.checkpoint={...current.queue.checkpoint,...payload.checkpoint};return current.queue;},resume:async payload=>{resumes+=1;current.campaign.status='running';current.queue.status='opening';current.source.status='opening';current.queue.checkpoint={...payload.checkpoint};return current.queue;},now:()=> '2026-08-31T00:00:00.000Z'},setPage:value=>{page=value;},setContext:value=>{current=structuredClone(value);},get submits(){return submits;},get resumes(){return resumes;},get writes(){return writes;}};}
