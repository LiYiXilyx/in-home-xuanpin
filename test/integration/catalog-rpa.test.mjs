...
test('Scale Day3 Catalog RPA queue claims, checkpoints, manual-resumes and completes without duplicate staging or Product Pool writes',async t => {
  ...
  const passiveCampaign=app.catalogService.createCampaign({ name:'manual-passive-guard',campaignType:'smoke',profile,browserContext:{ profileName:'Temu1店',profileDirectory:'Profile 10',controlMode:'MANUAL_BIND_PASSIVE_CAPTURE' } });
  ...
  const unbound=passiveBatch(passiveCampaign.id,passiveSource.id,'manual-unbound');delete unbound.page_binding;const beforeUnbound=writeCounts(app.db);
  call=await post('/api/catalog/batches',unbound);assert.equal(call.response.status,400);assert.equal(call.body.error.code,'PAGE_BINDING_REQUIRED');assert.deepEqual(writeCounts(app.db),beforeUnbound);
  ...
  const afterManual=writeCounts(app.db);call=await post('/api/catalog/batches',passiveBatch(passiveCampaign.id,passiveSource.id,'manual-valid-proof'));
  assert.equal(call.response.status,200);assert.equal(call.body.result.idempotentReplay,true);assert.deepEqual(writeCounts(app.db),afterManual);
  const passiveContext=app.catalogService.getCaptureContext(passiveCampaign.id,passiveSource.id);
  assert.equal(passiveContext.campaign.browserControlMode,'MANUAL_BIND_PASSIVE_CAPTURE');assert.equal(passiveContext.campaign.browserProfileDirectory,'Profile 10');
...
function passiveBatch(campaignId,sourceId,batchId,{captureMode='MANUAL_BIND_PASSIVE_CAPTURE',networkObserved=true}={}) { const payload=batch(campaignId,sourceId,batchId),binding={ status:'BOUND',binding_version:'manual-bind-v1',binding_generation:1,campaign_id:campaignId,source_id:sourceId,category_key:'motorcycle-accessories',category_profile_version:'motorcycle-accessories-v1',site_country:'DE',language:'en',currency:'EUR',sort_order:'Top Sales',bound_url:payload.page_url,bound_at:'2026-08-26T05:59:00.000Z',bound_category:'Motorcycles & Powersports Accessories',bound_sort:'Top Sales',bound_goods_count:1 };binding.context_fingerprint=fingerprint([binding.bound_url,binding.site_country,binding.language,binding.currency,binding.category_key,binding.bound_category,binding.bound_sort]);payload.capture_mode=captureMode;payload.page_binding=binding;payload.cards=[{ ...card('9001','Mechanical Tail Bag',1),capture_transport:'NETWORK_ENRICHED',network_observed:networkObserved,network_endpoint:'/api/poppy/v1/opt',network_observed_at:'2026-08-26T06:00:00.000Z',bound_url:binding.bound_url,bound_at:binding.bound_at,bound_category:binding.bound_category,bound_sort:binding.bound_sort,field_provenance:{ goods_id:'identity',source_url:'dom',title:'network' } }];return payload; }
...
function writeCounts(db){return Object.fromEntries(['catalog_capture_batches','catalog_product_source_observations','catalog_staging_products','catalog_exclusion_observations'].map(table=>[table,count(db,table)]));}
