...
export async function exportCatalogExpansionWorkbook(db,{ campaignId,outputDir }) {
  ...
  const workbook=buildCatalogExpansionWorkbook(artifact,model,{ clickableLinks:true }).workbook;
...
export function buildCatalogExpansionReportModel(db,campaignId) {
  ...
  const currentPool=db.prepare(`SELECT i.goods_id,s.latest_title,s.canonical_url,s.latest_source_url,s.image_url,s.price_amount,s.currency,
      s.sales_count,s.rating,s.review_count,i.membership_status,s.last_seen_at
...
export function buildCatalogExpansionWorkbook({ Workbook },model,{ clickableLinks=true }={}) {
  ...
  productsSheet(sheets['当前1500商品池'],model.currentPool,'CatalogExpansionPool',clickableLinks);
  newItemsSheet(sheets['本轮新增500'],model.newItems,clickableLinks);stagingSheet(sheets.Staging,model.staging,clickableLinks);
  exclusionSheet(sheets['电子排除'],model);sourceSheet(sheets['来源贡献'],model);qualitySheet(sheets['数据质量'],model);
  campaignSheet(sheets['Campaign任务'],model);fieldsSheet(sheets['字段说明']);return { workbook,sheetNames:SHEETS };
}

function productsSheet(sheet,items,tableName,clickable) {
  const headers=['序号','goods_id','商品标题','商品链接','图片链接','价格','币种','销量','评分','评论数','Pool状态','最后观察'];
  const rows=items.map((r,i)=>[i+1,`'${r.goods_id}`,r.latest_title,null,r.image_url,r.price_amount,r.currency,r.sales_count,r.rating,r.review_count,r.membership_status,toDate(r.last_seen_at)]);
  writeTable(sheet,headers,rows,tableName,[7,20,52,60,54,12,9,12,9,12,14,22]);writeLinks(sheet,'D',items.map(r=>r.canonical_url||r.latest_source_url),2,clickable);formatProducts(sheet,rows.length,'L');
}
function newItemsSheet(sheet,items,clickable) {
  const headers=['序号','goods_id','商品标题','商品链接','图片链接','价格','币种','销量','评分','评论数','首次来源','首次顺序','最后观察'];
  const rows=items.map((r,i)=>[i+1,`'${r.goods_id}`,r.latest_title,null,r.image_url,r.price_amount,r.currency,r.sales_count,r.rating,r.review_count,r.first_source_id,r.first_seen_sequence,toDate(r.last_seen_at)]);
  writeTable(sheet,headers,rows,'CatalogExpansionNew500',[7,20,52,60,54,12,9,12,9,12,38,11,22]);writeLinks(sheet,'D',items.map(r=>r.canonical_url||r.latest_source_url),2,clickable);formatProducts(sheet,rows.length,'M');
}
function stagingSheet(sheet,items,clickable) {
  const headers=['序号','goods_id','商品标题','商品链接','价格','币种','销量','评分','评论数','电子筛选','扩容状态','首次来源','首次顺序','最后观察'];
  const rows=items.map((r,i)=>[i+1,`'${r.goods_id}`,r.latest_title,null,r.price_amount,r.currency,r.sales_count,r.rating,r.review_count,r.electronic_screening_status,r.expansion_status,r.first_source_id,r.first_seen_sequence,toDate(r.last_seen_at)]);
  writeTable(sheet,headers,rows,'CatalogExpansionStaging',[7,20,52,60,12,9,12,9,12,22,20,38,11,22]);writeLinks(sheet,'D',items.map(r=>r.canonical_url||r.latest_source_url),2,clickable);
  if(rows.length)sheet.getRange(`E2:E${rows.length+1}`).format.numberFormat='€#,##0.00';if(rows.length)sheet.getRange(`N2:N${rows.length+1}`).format.numberFormat='yyyy-mm-dd hh:mm';
}
...
function fieldsSheet(sheet){const rows=[['platform + goods_id','跨batch/source/campaign稳定身份','SQLite','URL不作为身份键'],['baseline_count','激活前正式1000 Pool','SQLite','扩容全过程冻结'],['new_non_electronic_unique','相对baseline净新增passed商品','Staging','目标500'],['manual_review_required','需人工判断','Staging','不计1500 Gate'],['exclusion_code','电子硬排除审计','Exclusion audit','不得进入Pool'],['Pool Version','baseline与新增的事务合并版本','SQLite','1500激活后1000 superseded'],['source_url / canonical_url','历史证据与fallback','SQLite/页面','不是长期可靠详情地址'],['Excel','运营查看与交接','Artifact','SQLite是唯一正式数据源']];writeTable(sheet,['字段','定义','来源','口径说明'],rows,'CatalogExpansionFields',[38,52,24,66]);}
...
function formatProducts(sheet,count,dateColumn){if(!count)return;const last=count+1;sheet.getRange(`F2:F${last}`).format.numberFormat='€#,##0.00';sheet.getRange(`H2:H${last}`).format.numberFormat='#,##0';sheet.getRange(`I2:I${last}`).format.numberFormat='0.0';sheet.getRange(`J2:J${last}`).format.numberFormat='#,##0';sheet.getRange(`${dateColumn}2:${dateColumn}${last}`).format.numberFormat='yyyy-mm-dd hh:mm';sheet.getRange(`C2:E${last}`).format.wrapText=true;sheet.freezePanes.freezeColumns(2);}
