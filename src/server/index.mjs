...
export async function createOperationsServer(options={}) {
  ...
  const statusService=createStatusService({ db,jobRepository:repository,config,browserStatus:() => browserController.status(),latestExcel:exportController.latestExcel });
  const serveStatic=createStaticServer(path.join(projectDir,'ui'));
  const router=createRouter({ statusService,browserController,jobController,exportController,serveStatic,logError:options.logError });
