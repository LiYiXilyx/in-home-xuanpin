...
export function createOpportunityAnalysisRepository(db,{ now=()=>new Date().toISOString() }={}) {
  ...
  function createSnapshot({ sourcePoolVersionId,sourceCampaignId,siteCountry='DE',language='en',currency='EUR',sortContext='Top Sales',config={} }) {
    ...
    if(!pool || pool.status!=='ready')throw new Error('Opportunity Snapshot要求ready Source Pool。');
    const existing=db.prepare('SELECT * FROM opportunity_analysis_snapshots WHERE source_pool_version_id=?').get(sourcePoolVersionId);
  ...
  function updateItemAnalysis(snapshotId,goodsId,result){db.prepare(`UPDATE opportunity_snapshot_items SET included=?,data_quality_json=?,hard_exclusion_codes_json=?,warning_codes_json=?,
    ...
    classification_reasons_json=?,manual_review_required=? WHERE snapshot_id=? AND goods_id=?`).run(
      ...
      result.confidence,JSON.stringify(result.reasons),result.manualReviewRequired?1:0,snapshotId,goodsId);}
