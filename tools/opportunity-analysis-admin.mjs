...
try{const service=createOpportunityAnalysisService(db);if(action==='freeze-analyze'){const result=service.freezeAndAnalyze(required(options.campaign,'campaign'));const qa=await exportOpportunityWorkbook(result,{outputDir:path.resolve(options.output??'outputs/opportunity-analysis-current-pool')});print({snapshot:result.snapshot,summary:result.summary,candidates:result.candidates,excel:qa});}else if(action==='reanalyze'){const result=service.reanalyze(options.snapshot??null);print({snapshot:result.snapshot,summary:result.summary,candidates:result.candidates});}else if(action==='status'){const result=service.getResult(options.snapshot??null);print({snapshot:result.snapshot,summary:result.summary,candidates:result.candidates,coreCounts:result.coreCounts});}else if(action==='excel'){const result=service.getResult(options.snapshot??null);print(await exportOpportunityWorkbook(result,{outputDir:path.resolve(options.output??'outputs/opportunity-analysis-current-pool')}));}else throw new Error('未知操作：freeze-analyze/reanalyze/status/excel');}finally{db.close();}
